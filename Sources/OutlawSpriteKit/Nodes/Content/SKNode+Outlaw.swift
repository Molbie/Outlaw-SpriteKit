...
public extension SKNode {
    ...
    private typealias keys = SKNode.NodeExtractableKeys
}
...
public extension SKNode { /* Serializable */
    func serializedNode(withChildren: Bool) -> [String: Any] {
...
public extension SKNode { /* Updatable */
    func updateNode(with object: Extractable) throws {
        if let position: CGPoint = object.optional(for: keys.position) {
            self.position = position
        }
        if let zPosition: CGFloat = object.optional(for: keys.zPosition) {
            self.zPosition = zPosition
        }
        if let zRotation: CGFloat = object.optional(for: keys.zRotation) {
            self.zRotation = zRotation
        }
        if let xScale: CGFloat = object.optional(for: keys.xScale) {
            self.xScale = xScale
        }
        if let yScale: CGFloat = object.optional(for: keys.yScale) {
            self.yScale = yScale
        }
        if let speed: CGFloat = object.optional(for: keys.speed) {
            self.speed = speed
        }
        if let alpha: CGFloat = object.optional(for: keys.alpha) {
            self.alpha = alpha
        }
        if let isPaused: Bool = object.optional(for: keys.isPaused) {
            self.isPaused = isPaused
        }
        if let isHidden: Bool = object.optional(for: keys.isHidden) {
            self.isHidden = isHidden
        }
        if let isUserInteractionEnabled: Bool = object.optional(for: keys.isUserInteractionEnabled) {
...
//
//        }
        if let name: String = object.optional(for: keys.name) {
            self.name = name
        }
        if let physicsBodyData: [String: Any] = object.optional(for: keys.physicsBody) {
            // TODO: create a physics body if one doesn't already exist
            if let physicsBody = self.physicsBody {
                try SKPhysicsBody.update(physicsBody, with: physicsBodyData)
            }
        }
        if let userData: [AnyHashable: Any] = object.optional(for: keys.userData) {
        ...
        if #available(OSX 10.10, *) {
            if let reachConstraints: [String: Any] = object.optional(for: keys.reachConstraints) {
