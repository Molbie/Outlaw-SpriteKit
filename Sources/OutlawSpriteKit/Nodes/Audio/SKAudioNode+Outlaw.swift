...
public extension SKAudioNode {
    struct AudioNodeExtractableKeys {
    ...
    private typealias keys = SKAudioNode.AudioNodeExtractableKeys
}

@available(iOS 9.0, OSX 10.11, *)
public extension SKAudioNode { /* Serializable */
    func serializedAudioNode(withChildren: Bool) -> [String: Any] {
...
public extension SKAudioNode { /* Updatable */
    func updateAudioNode(with object: Extractable) throws {
...
//        }
        if let autoplayLooped: Bool = object.optional(for: keys.autoplayLooped) {
            self.autoplayLooped = autoplayLooped
        }
        if let isPositional: Bool = object.optional(for: keys.isPositional) {
