...
public extension SKShapeNode {
    struct ShapeNodeExtractableKeys {
    ...
    private typealias keys = SKShapeNode.ShapeNodeExtractableKeys
}


public extension SKShapeNode { /* Serializable */
    func serializedShapeNode(withChildren: Bool) -> [String: Any] {
...
public extension SKShapeNode { /* Updatable */
    func updateShapeNode(with object: Extractable) throws {
        try self.updateNode(with: object)
// TODO: update path
//        if let path: CGPath = object.value(for: keys.path) {
//            self.path = path
//        }
        if let strokeColor: SKColor = object.optional(for: keys.strokeColor) {
            self.strokeColor = strokeColor
        }
        if let fillColor: SKColor = object.optional(for: keys.fillColor) {
            self.fillColor = fillColor
        }
        if let stringValue: String = object.optional(for: keys.blendMode), let blendMode = SKBlendMode(stringValue: stringValue) {
            self.blendMode = blendMode
        }
        if let isAntialiased: Bool = object.optional(for: keys.isAntialiased) {
            self.isAntialiased = isAntialiased
        }
        if let lineWidth: CGFloat = object.optional(for: keys.lineWidth) {
            self.lineWidth = lineWidth
        }
        if let glowWidth: CGFloat = object.optional(for: keys.glowWidth) {
            self.glowWidth = glowWidth
        }
        if let stringValue: String = object.optional(for: keys.lineCap), let lineCap = CGLineCap(stringValue: stringValue) {
            self.lineCap = lineCap
        }
        if let stringValue: String = object.optional(for: keys.lineJoin), let lineJoin = CGLineJoin(stringValue: stringValue) {
            self.lineJoin = lineJoin
        }
        if let miterLimit: CGFloat = object.optional(for: keys.miterLimit) {
