...
public extension SKEmitterNode {
    struct EmitterNodeExtractableKeys {
...
public extension SKEmitterNode { /* Serializable */
    func serializedEmitterNode(withChildren: Bool) -> [String: Any] {
...
public extension SKEmitterNode { /* Updatable */
    func updateEmitterNode(with object: Extractable) throws {
...
//
//        }
        if let stringValue: String = object.optional(for: keys.particleBlendMode), let particleBlendMode = SKBlendMode(stringValue: stringValue) {
            self.particleBlendMode = particleBlendMode
        }
        if let particleColor: SKColor = object.optional(for: keys.particleColor) {
            self.particleColor = particleColor
        }
        if let particleColorRedRange: CGFloat = object.optional(for: keys.particleColorRedRange) {
            self.particleColorRedRange = particleColorRedRange
        }
        if let particleColorGreenRange: CGFloat = object.optional(for: keys.particleColorGreenRange) {
            self.particleColorGreenRange = particleColorGreenRange
        }
        if let particleColorBlueRange: CGFloat = object.optional(for: keys.particleColorBlueRange) {
            self.particleColorBlueRange = particleColorBlueRange
        }
        if let particleColorAlphaRange: CGFloat = object.optional(for: keys.particleColorAlphaRange) {
            self.particleColorAlphaRange = particleColorAlphaRange
        }
        if let particleColorRedSpeed: CGFloat = object.optional(for: keys.particleColorRedSpeed) {
            self.particleColorRedSpeed = particleColorRedSpeed
        }
        if let particleColorGreenSpeed: CGFloat = object.optional(for: keys.particleColorGreenSpeed) {
            self.particleColorGreenSpeed = particleColorGreenSpeed
        }
        if let particleColorBlueSpeed: CGFloat = object.optional(for: keys.particleColorBlueSpeed) {
            self.particleColorBlueSpeed = particleColorBlueSpeed
        }
        if let particleColorAlphaSpeed: CGFloat = object.optional(for: keys.particleColorAlphaSpeed) {
            self.particleColorAlphaSpeed = particleColorAlphaSpeed
        }
        if let particleColorSequence: [String: Any] = object.optional(for: keys.particleColorSequence) {
            // TODO: create one if it doesn't already exist
            try self.particleColorSequence?.update(with: particleColorSequence)
        }
        if let particleColorBlendFactor: CGFloat = object.optional(for: keys.particleColorBlendFactor) {
            self.particleColorBlendFactor = particleColorBlendFactor
        }
        if let particleColorBlendFactorRange: CGFloat = object.optional(for: keys.particleColorBlendFactorRange) {
            self.particleColorBlendFactorRange = particleColorBlendFactorRange
        }
        if let particleColorBlendFactorSpeed: CGFloat = object.optional(for: keys.particleColorBlendFactorSpeed) {
            self.particleColorBlendFactorSpeed = particleColorBlendFactorSpeed
        }
        if let particleColorBlendFactorSequence: [String: Any] = object.optional(for: keys.particleColorBlendFactorSequence) {
            // TODO: create one if it doesn't already exist
            try self.particleColorBlendFactorSequence?.update(with: particleColorBlendFactorSequence)
        }
        if let particlePosition: CGPoint = object.optional(for: keys.particlePosition) {
            self.particlePosition = particlePosition
        }
        if let particlePositionRange: CGVector = object.optional(for: keys.particlePositionRange) {
            self.particlePositionRange = particlePositionRange
        }
        if let particleSpeed: CGFloat = object.optional(for: keys.particleSpeed) {
            self.particleSpeed = particleSpeed
        }
        if let particleSpeedRange: CGFloat = object.optional(for: keys.particleSpeedRange) {
            self.particleSpeedRange = particleSpeedRange
        }
        if let emissionAngle: CGFloat = object.optional(for: keys.emissionAngle) {
            self.emissionAngle = emissionAngle
        }
        if let emissionAngleRange: CGFloat = object.optional(for: keys.emissionAngleRange) {
            self.emissionAngleRange = emissionAngleRange
        }
        if let xAcceleration: CGFloat = object.optional(for: keys.xAcceleration) {
            self.xAcceleration = xAcceleration
        }
        if let yAcceleration: CGFloat = object.optional(for: keys.yAcceleration) {
            self.yAcceleration = yAcceleration
        }
        if let particleBirthRate: CGFloat = object.optional(for: keys.particleBirthRate) {
            self.particleBirthRate = particleBirthRate
        }
        if let numParticlesToEmit: Int = object.optional(for: keys.numParticlesToEmit) {
            self.numParticlesToEmit = numParticlesToEmit
        }
        if let particleLifetime: CGFloat = object.optional(for: keys.particleLifetime) {
            self.particleLifetime = particleLifetime
        }
        if let particleLifetimeRange: CGFloat = object.optional(for: keys.particleLifetimeRange) {
            self.particleLifetimeRange = particleLifetimeRange
        }
        if let particleRotation: CGFloat = object.optional(for: keys.particleRotation) {
            self.particleRotation = particleRotation
        }
        if let particleRotationRange: CGFloat = object.optional(for: keys.particleRotationRange) {
            self.particleRotationRange = particleRotationRange
        }
        if let particleRotationSpeed: CGFloat = object.optional(for: keys.particleRotationSpeed) {
            self.particleRotationSpeed = particleRotationSpeed
        }
        if let particleSize: CGSize = object.optional(for: keys.particleSize) {
            self.particleSize = particleSize
        }
        if let particleScale: CGFloat = object.optional(for: keys.particleScale) {
            self.particleScale = particleScale
        }
        if let particleScaleRange: CGFloat = object.optional(for: keys.particleScaleRange) {
            self.particleScaleRange = particleScaleRange
        }
        if let particleScaleSpeed: CGFloat = object.optional(for: keys.particleScaleSpeed) {
            self.particleScaleSpeed = particleScaleSpeed
        }
        if let particleScaleSequence: [String: Any] = object.optional(for: keys.particleScaleSequence) {
            // TODO: create one if it doesn't already exist
            try self.particleScaleSequence?.update(with: particleScaleSequence)
        }
        if let particleAlpha: CGFloat = object.optional(for: keys.particleAlpha) {
            self.particleAlpha = particleAlpha
        }
        if let particleAlphaRange: CGFloat = object.optional(for: keys.particleAlphaRange) {
            self.particleAlphaRange = particleAlphaRange
        }
        if let particleAlphaSpeed: CGFloat = object.optional(for: keys.particleAlphaSpeed) {
            self.particleAlphaSpeed = particleAlphaSpeed
        }
        if let particleAlphaSequence: [String: Any] = object.optional(for: keys.particleAlphaSequence) {
            // TODO: create one if it doesn't already exist
...
//            }
//        }
        if let particleZPosition: CGFloat = object.optional(for: keys.particleZPosition) {
            self.particleZPosition = particleZPosition
        }
        if #available(iOS 9.0, OSX 10.11, *) {
            if let stringValue: String = object.optional(for: keys.particleRenderOrder), let particleRenderOrder = SKParticleRenderOrder(stringValue: stringValue) {
                self.particleRenderOrder = particleRenderOrder
            }
        }
        else {
            if let particleZPositionRange: CGFloat = object.optional(for: keys.particleZPositionRange) {
                self.particleZPositionRange = particleZPositionRange
            }
            if let particleZPositionSpeed: CGFloat = object.optional(for: keys.particleZPositionSpeed) {
