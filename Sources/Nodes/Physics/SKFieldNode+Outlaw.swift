...
public extension SKFieldNode {
    public struct FieldNodeExtractableKeys {
    ...
    fileprivate typealias keys = SKFieldNode.FieldNodeExtractableKeys
}

@available(OSX 10.10, *)
public extension SKFieldNode { /* Serializable */
    public func serializedFieldNode(withChildren: Bool) -> [String: Any] {
...
public extension SKFieldNode { /* Updatable */
    public func updateFieldNode(with object: Extractable) throws {
        try self.updateNode(with: object)
// TODO: update region
//        if let region: SKRegion = object.value(for: keys.region) {
//            self.region = region
//        }
        if let strength: Float = object.value(for: keys.strength) {
            self.strength = strength
        }
        if let falloff: Float = object.value(for: keys.falloff) {
            self.falloff = falloff
        }
        if let minimumRadius: Float = object.value(for: keys.minimumRadius) {
            self.minimumRadius = minimumRadius
        }
        if let isEnabled: Bool = object.value(for: keys.isEnabled) {
            self.isEnabled = isEnabled
        }
        if let isExclusive: Bool = object.value(for: keys.isExclusive) {
            self.isExclusive = isExclusive
        }
        if let categoryBitMask: UInt32 = object.value(for: keys.categoryBitMask) {
            self.categoryBitMask = categoryBitMask
        }
        if let direction: vector_float3 = object.value(for: keys.direction) {
            self.direction = direction
        }
        if let smoothness: Float = object.value(for: keys.smoothness) {
            self.smoothness = smoothness
        }
        if let animationSpeed: Float = object.value(for: keys.animationSpeed) {
