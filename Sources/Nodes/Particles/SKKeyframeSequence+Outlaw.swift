...
public extension SKKeyframeSequence {
    ...
    fileprivate typealias keys = SKKeyframeSequence.ExtractableKeys
}
...
extension SKKeyframeSequence: Updatable {
    public func update(with object: Extractable) throws {
        if let stringValue: String = object.value(for: keys.interpolationMode), let interpolationMode = SKInterpolationMode(stringValue: stringValue) {
            self.interpolationMode = interpolationMode
        }
        if let stringValue: String = object.value(for: keys.repeatMode), let repeatMode = SKRepeatMode(stringValue: stringValue) {
